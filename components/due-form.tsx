...
export function DueForm({ onSubmit, isSubmitting, initialData, isEditing = false }: DueFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        recurrence_frequency: initialData.recurrence_frequency?.toString() || "1",
  ...
  const handleSubmit = (e) => {
    ...
    const formattedData = {
      ...
      due_day: formData.due_day ? Number.parseInt(formData.due_day) : new Date(formData.start_date).getDate(),
      recurrence_frequency: Number.parseInt(formData.recurrence_frequency),
      occurrences: formData.end_type === "after_occurrences" ? Number.parseInt(formData.occurrences) : undefined,
      end_date: formData.end_type === "after_date" ? formData.end_date : undefined,
    }
  ...
  const safeSetDayOfMonth = (date, day) => {
    try {
  ...
  const getRecurrenceText = () => {
    ...
    switch (formData.recurrence) {
      ...
      case "biweekly":
        return `${baseText}${frequency === 1 ? "Every 2 weeks" : "bi-weeks"}`
      case "monthly":
        return `${baseText}${frequency === 1 ? "Monthly" : "months"}`
      case "quarterly":
        return `${baseText}${frequency === 1 ? "Every 3 months" : "quarters"}`
      case "annually":
  ...
  const getNextDueDates = () => {
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (formData.due_day && (formData.recurrence === "monthly" || formData.recurrence === "quarterly")) {
          currentDate = safeSetDayOfMonth(currentDate, formData.due_day)
        }
  ...
  const showDueDay = formData.recurrence === "monthly" || formData.recurrence === "quarterly"
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {/* Basic Recurrence */}
          <div className="space-y-2">
            <Label htmlFor="recurrence">How often does this bill repeat?</Label>
          ...
          {showCustomFrequency && (
            <div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
              <Label htmlFor="recurrence_frequency">Custom frequency (optional)</Label>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {formData.recurrence === "weekly"
                    ? Number.parseInt(formData.recurrence_frequency || "1") === 1
                      ? "week"
                      : "weeks"
          ...
          {showDueDay && (
            <div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
              <Label htmlFor="due_day">Specific day of the month (optional)</Label>
              <Input
                id="due_day"
                name="due_day"
                type="number"
              ...
              <p className="text-xs text-muted-foreground">
                Leave blank to use the day from your first due date ({safeCreateDate(formData.start_date).getDate()})
              </p>
            </div>
          )}
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground">
                <p className="font-medium mb-1">Next 3 due dates:</p>
                <div className="space-y-1">
                  ...
                  {getNextDueDates().length === 0 && (
                    <div className="flex items-center gap-2 text-amber-600">
                      ...
                      <span>Unable to calculate dates - please check your settings</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <Label className="text-base font-medium">When should this bill stop?</Label>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">This bill will continue indefinitely</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 space-y-2">
                  <Label htmlFor="end_after_date" className="font-normal cursor-pointer">
              ...
              <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                ...
                />
                <div className="flex-1 space-y-2">
                  <Label htmlFor="end_after_occurrences" className="font-normal cursor-pointer">
      ...
      <div className="flex justify-end space-x-4">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? "Saving..." : isEditing ? "Update Due" : "Create Due"}
        </Button>
      </div>
    </form>
  )
