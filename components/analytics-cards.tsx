...
export function AnalyticsCards({ dueInstances }: AnalyticsCardsProps) {
  ...
  // Calculate analytics
  const analytics = {
    totalMonthlyAmount: currentMonthDues.reduce((sum, due) => sum + due.monthly_dues.amount, 0),

    paidAmount: currentMonthDues.filter((due) => due.is_paid).reduce((sum, due) => sum + due.monthly_dues.amount, 0),
  ...
  const paidBillsThisMonth = currentMonthDues.filter((due) => due.is_paid).length
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {paidBillsThisMonth} of {totalBillsThisMonth} bills paid this month
          </p>
        </CardContent>
      </Card>
