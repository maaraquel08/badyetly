...
export default function EditDuePage({ params }: EditDuePageProps) {
    ...
    const fetchDue = async () => {
      ...
      if (!uuidRegex.test(id)) {
        ...
        toast({
          ...
          description: "The due ID format is invalid. Please check the URL.",
          variant: "destructive",
        })
      ...
      try {
        ...
        if (error) {
          if (error.code === "PGRST116") {
            ...
            toast({
              title: "Due not found",
              description: "The requested due could not be found or you don't have permission to access it.",
              variant: "destructive",
            })
          } else {
            throw error
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      await supabase.from("due_instances").delete().eq("monthly_due_id", id).gt("due_date", today).is("is_paid", false)
      ...
      let instanceCount = 0
      const maxInstances = formData.end_type === "after_occurrences" ? Number.parseInt(formData.occurrences) : 100 // Limit to 100 instances for "never" end type
      const endDate = formData.end_type === "after_date" ? new Date(formData.end_date) : new Date(9999, 11, 31) // Far future date for "never" end type
      ...
      while (instanceCount < maxInstances && currentDate <= endDate) {
        ...
        switch (formData.recurrence) {
          ...
          case "monthly":
            ...
            break
          case "quarterly":
            currentDate.setMonth(currentDate.getMonth() + 3 * frequency)
            break
          case "annually":
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await supabase.from("due_instances").delete().eq("monthly_due_id", id)

      // Then delete the monthly due
      const { error } = await supabase.from("monthly_dues").delete().eq("id", id).eq("user_id", user.id) // Ensure user can only delete their own dues
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <DashboardHeader heading="Edit Due" text="Loading due details..." />
        <div className="flex items-center justify-center h-64">
          <p className="text-muted-foreground">Loading...</p>
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <DashboardHeader heading="Edit Due" text="There was an error loading the due details." />
        <div className="flex items-center justify-center h-64 flex-col">
          <p className="text-red-500 mb-4">Error: {error}</p>
          <Button onClick={() => router.push("/dashboard/dues")}>Back to Dues</Button>
        </div>
      </div>
    )
  }
  ...
  if (!due) {
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-center h-64 flex-col">
          <p className="text-muted-foreground mb-4">The requested due could not be found.</p>
          <Button onClick={() => router.push("/dashboard/dues")}>Back to Dues</Button>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <DashboardHeader heading="Edit Due" text="Update your recurring monthly payment.">
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete this monthly due and all its payment history. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <DueForm onSubmit={handleSubmit} isSubmitting={isSubmitting} initialData={due} isEditing={true} />
    </div>
  )
}
