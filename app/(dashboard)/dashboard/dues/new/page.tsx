...
export default function NewDuePage() {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      console.log("Creating monthly due with user ID:", user.id)
      ...
      if (monthlyDueError) {
        console.error("Error creating monthly due:", monthlyDueError)
        throw monthlyDueError
      }

      console.log("Monthly due created:", monthlyDue)
      ...
      console.log(`Generated ${dueInstances.length} due instances`)

      // Insert due instances
      const { error: instancesError } = await supabase.from("due_instances").insert(dueInstances)

      if (instancesError) {
        console.error("Error creating due instances:", instancesError)
      ...
      toast({
        ...
        description: error.message || "There was an error creating your monthly due. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  if (isPageLoading || authLoading) {
    return (
      <div className="space-y-6">
        <DashboardHeader heading="Add New Due" text="Loading..." />
  ...
  if (!user) {
    return (
      <div className="space-y-6">
        <DashboardHeader heading="Authentication Required" text="Please log in to create a new due." />
  ...
  return (
    <div className="space-y-6">
      <DashboardHeader heading="Add New Due" text="Create a new recurring monthly payment." />
